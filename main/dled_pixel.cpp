...
void dled_pixel_chase_pixels(pixel_t *pixels, uint16_t length, uint8_t max_cc_val, uint16_t step, uint16_t num) {
    ...
    for (uint16_t i = 0; i < length; i++) {
        ...
        } else { // Gradually make the LED dimmer and dimmer with each step (falling behind)
            if (step >= num) {
                pixels[i].r /= 2;
                pixels[i].g /= 2;
                pixels[i].b /= 2;
...
// //         pixels[k] = pixels[k-1]; // Shift everything by one
// //         if (k < num) {
// //
// //         }
// //     }
//     for (int k = i; k < numItems; k++) // shift the array values from point i
//         memmove(&pixels[k+1], &pixels[k], (length-k-1)*sizeof(double));
//         pixels[i] = pixel;
//     }
//
//
// //     for (uint16_t i = 0; i < length-1; i++) {
// //         pixels[k+1] = pixels[k]; // Shift everything by one
// //     }
// }
