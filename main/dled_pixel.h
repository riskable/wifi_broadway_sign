...
/**
 * @brief Moves a number of pixels back and forth
 *
 * Moves a number of pixels back and forth changing its color through red, green and blue.
 * There are (6 * length) steps.
 *
 * @param[in,out] pixels     The pixels to be set.
 * @param[in]     length     Number of pixels.
 * @param[in]     max_cc_val The maximum value allowed for a color component.
 * @param[in]     step       The step of the moving sequence.
 * @param[in]     num        The number of pixels to move at a time.
 *
 * @code{c}
 * pixel_t *pixels;
 * ... // create the pixels
 * uint16_t step = 0;
 * while (true) {
 *     dled_pixel_chase_pixels(pixels, number_of_pixels, max_cc_val, step++, 3);
 *     ... // send the pixels to LEDs
 *     delay_ms(50); // wait a little between sequences
 * }
 * @endcode
 */
void dled_pixel_chase_pixels(pixel_t *pixels, uint16_t length, uint8_t max_cc_val, uint16_t step, uint16_t num);

void dled_pixel_chase_pixel_test(pixel_t *pixels, uint16_t length, uint8_t max_cc_val, uint16_t step, uint16_t num);
